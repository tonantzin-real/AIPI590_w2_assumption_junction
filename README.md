...
| Logistic regression | Accuracy = 0.803; F1 = 0.605; ROC-AUC = 0.836 | Coefficients turn directly into odds ratios, so we can explain each feature's effect on churn in plain terms (e.g., a one-year contract cuts the odds of churn roughly in half) | A few features (MonthlyCharges, InternetService) are highly correlated with each other, so their individual coefficients are unstable and shouldn't be trusted on their own |
